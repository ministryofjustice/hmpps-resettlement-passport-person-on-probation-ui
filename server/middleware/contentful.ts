...
export const fetchPage = async (locale: string, pageId: string) => {
  ...
  try {
    ...
    const contentArray = response.items.map(x => ({
      ...
      value: x.fields.bodyText.content.flatMap(content => content.content.map(c => c.value)).join(''),
    }))
...
const fetchContent = async (locale: string) => {
  ...
  try {
    ...
    const arrayJson = contentArray.reduce((acc, curr) => {
      return { ...acc, ...curr }
    }, {})
    fs.writeFile(templatesDir + getFileNameForLocale(locale), JSON.stringify(arrayJson), () => {})
  } catch (error) {
    console.error(error)
  }
}
