...
export async function returnSecurityCode(number) {
...
export async function returnCurrentCount() {
...
export async function countMessages(auth) {
  count = 0
  const gmail = google.gmail({ version: 'v1', auth })
  const res = await gmail.users.messages.list({
    userId: 'me',
...
export async function getMessage(auth) {
...
export async function extractSixDigitNumber(input) {
  const regex = /\b\d{6}\b/
